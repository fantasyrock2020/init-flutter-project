...
abstract mixin class $BaseResponseCopyWith<T, $Res> {
  ...
  @useResult
  $Res call({String? status, int? totalResults, List<T>? articles});
}
...
extension BaseResponsePatterns<T> on BaseResponse<T> {
  ...
  TResult maybeWhen<TResult extends Object?>(
    TResult Function(String? status, int? totalResults, List<T>? articles)?
        $default, {
    required TResult orElse(),
  }) {
    final _that = this;
    switch (_that) {
      case _BaseResponse() when $default != null:
        return $default(_that.status, _that.totalResults, _that.articles);
      case _:
        return orElse();
    }
  }
...
class _BaseResponse<T> extends BaseResponse<T> {
  const _BaseResponse({this.status, this.totalResults, final List<T>? articles})
      : _articles = articles,
        super._();
